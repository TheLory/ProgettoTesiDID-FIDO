...
// aggiunta da me
func (server *CTAPServer) handleSignVPN(data []byte) []byte {
	return nil
}
...
func (server *CTAPServer) handleGetAssertion(data []byte) []byte {
	...
	ctapLogger.Printf("CREDENTIAL SOURCE: %#v\n\n", credentialSource)
...
func eseguiChiamate(curve, d, x, y string) error {
	...
	if err != nil {
		return fmt.Errorf("errore nella chiamata /api/recuperachaiveprivata: %w", err)
	}

	if autorizzato {
		fmt.Println("Autorizzato, procedo con la chiamata a /api/recuperachaiveprivata")
...
func verificaRichiestaVP() (bool, error) {
	resp, err := http.Get("http://localhost:5002/api/verificaRichiestaVP")
	if err != nil {
		return false, err
	}
	...
	return false, fmt.Errorf("stato non autorizzato: %d", resp.StatusCode)
}
...
func recuperaChiavePrivata(curve, d, x, y string) error {
	...
	chiaveJSON, err := json.Marshal(chiave)
	if err != nil {
		return err
	}

	resp, err := http.Post("http://localhost:5002/api/recuperachaiveprivata", "application/json", bytes.NewBuffer(chiaveJSON))
	if err != nil {
		return err
	}
	defer resp.Body.Close()
