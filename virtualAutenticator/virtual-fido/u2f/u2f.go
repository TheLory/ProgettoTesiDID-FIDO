...
const (
	...
	u2f_COMMAND_SIGNVP       U2FCommand = 0x04
)
...
var U2FCommandDescriptions = map[U2FCommand]string{
	...
	u2f_COMMAND_SIGNVP:       "u2f_COMMAND_SIGNVP",
}
...
type U2FClient interface {
	...
	ApproveU2FVPSign(keyHandle *webauthn.KeyHandle) bool
}
...
func (server *U2FServer) HandleMessage(message []byte) []byte {
	...
	case u2f_COMMAND_SIGNVP:
		response = server.HandleApproveU2FVPSign(header, request)
	default:
...
// aggiunto da me
func (server *U2FServer) HandleApproveU2FVPSign(header U2FMessageHeader, request []byte) []byte {
	return nil
}
