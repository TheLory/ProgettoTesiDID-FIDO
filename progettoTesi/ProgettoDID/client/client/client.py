...
def genereateDidFromPEM():
    ...
    try:
        ...
        print("Chiave privata in formato JWK:", jwk_key)
...
async def issueVP():
    ...
    with open(vp_storage_file, 'r+') as file:
        ...
        file.truncate()
    print("FILE CREATO PORCO DIOOOOO")
    return presentation
...
async def verify_vp():
    ...
    public_key = pem_to_jwk_public_only(private_key_file_path)  

    verification_method = await didkit.key_to_verification_method("key",public_key)       
    ...
    validation_result = await didkit.verify_presentation(json.dumps(vp),json.dumps(options))
